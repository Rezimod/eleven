...
export function WinnerBanner({
  ...
  const yourPayout = payouts.find((p) => p.player === "You");
  return (
    <div className="card-accent p-6 text-center">
      ...
      {youWon && yourPayout && (
        <div className="num mt-1 text-2xl font-bold text-lime">You win {fmtUsd(yourPayout.amount)}</div>
      )}
      <div className="mt-3 flex flex-col gap-1">
        {payouts.map((p) => (
          <div key={p.player} className="flex items-center justify-between text-sm">
            ...
            <span className="num text-text">{fmtUsd(p.amount)}</span>
          </div>
        ))}
        <div className="mt-1 flex items-center justify-between text-xs text-faint">
          <span>house fee</span>
          <span className="num">{fmtUsd(rake)}</span>
        </div>
      </div>
    </div>
  );
